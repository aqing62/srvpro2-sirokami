...
import { LadderService } from './ladder';
...
export const FeatsModule = createAppContext()
  ...
  .provide(UserService) // user account CRUD (DB with file fallback)
  .provide(LoginService) // /login password authentication
  .provide(LadderService) // ELO ladder ranking system
